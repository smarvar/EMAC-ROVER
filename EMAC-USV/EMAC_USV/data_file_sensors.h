...
struct Sensors_data{
      float gps_latitude, gps_longitude, gps_altitude, gps_speed, gps_course, sens_0, sens_1, sens_2, sens_3, sens_4, sens_5, sens_6, sens_7, sens_8, angle_sonar;
...
void Pulse_counter(){
  digital_counter += 1;
  USV_data.sens_8 = digital_counter;
 }
...
void Data_sensors (){
     ...
     for (int ch = 0; ch < analog_sensors; ch += 1) {
       // String sensor_channel = Scan_file(Conf_file, channel[ch]);
       // if (sensor_channel != "0"){
        ...
        // if channel is equal to zero, then to write "0" in this sensor position and space in the sensor value
        else{
           switch (ch) {
                case 0:
                  USV_data.sens_0 = 0;
